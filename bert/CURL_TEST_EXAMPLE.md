# Curl Test Example for Role Detection
# Ví dụ Test Curl cho Role Detection
...
## Expected Response / Phản hồi Dự kiến
...
## Paragraph Analysis / Phân tích Đoạn văn

### System Messages / Thông báo Hệ thống (narrator)
- Paragraphs 0-12: System notifications `[ hệ thống: ...]` → Should be `narrator`
- Paragraph 13: Narration describing character's state → Should be `narrator`

### Character Dialogue / Lời nhân vật
- Paragraph 14: `"A, cứ như vậy đi..."` → Should be `male` (protagonist speaking)

### Narration / Dẫn chuyện
- Paragraphs 15-16: Third-person narration → Should be `narrator`

## Quick Test / Test Nhanh

### Using PowerShell Script / Sử dụng PowerShell Script

```powershell
cd bert
.\test_curl_role_detection.ps1
```

### Using Bash Script / Sử dụng Bash Script

```bash
cd bert
chmod +x test_curl_role_detection.sh
./test_curl_role_detection.sh
```

### Direct curl / Curl trực tiếp

Copy command from above and run in terminal.

## Notes / Lưu ý

1. **Backend must be running** on `http://localhost:11110`
2. **Ollama must be running** with `qwen3:8b` model
3. **Response time**: May take 10-30 seconds for 17 paragraphs
4. **Expected roles**:
   - System messages → `narrator`
   - Character dialogue → `male` or `female` (based on context)
   - Third-person narration → `narrator`
