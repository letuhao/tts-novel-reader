# Restart All Servers (VieNeu-TTS Backend + Frontend)
# Khởi động lại Tất cả Server (VieNeu-TTS Backend + Frontend)

Write-Host "=== Restarting All Servers (VieNeu-TTS Backend + Frontend) ===" -ForegroundColor Cyan
Write-Host "=== Khởi động lại Tất cả Server (VieNeu-TTS Backend + Frontend) ===" -ForegroundColor Cyan
Write-Host ""

# Stop VieNeu-TTS Backend
...
# Start Novel Backend with vieneu-tts model
Write-Host ""
Write-Host "Starting Novel Backend with VieNeu-TTS model..." -ForegroundColor Cyan
...
Write-Host "   ✅ Set TTS_DEFAULT_MODEL=vieneu-tts" -ForegroundColor Green
Write-Host "   ✅ Đã thiết lập TTS_DEFAULT_MODEL=vieneu-tts" -ForegroundColor Green
...
$tts = try {
    $response = Invoke-WebRequest -Uri "http://127.0.0.1:11111/api/tts/health" -TimeoutSec 2 -UseBasicParsing
    $true
} catch {
    $false
}
...
Write-Host ""
Write-Host "📝 Note: Using VieNeu-TTS Backend (vieneu-tts-backend)" -ForegroundColor Cyan
Write-Host "📝 Lưu ý: Đang sử dụng VieNeu-TTS Backend (vieneu-tts-backend)" -ForegroundColor Cyan
Write-Host ""
