...
if ($existingProcess) {
    Write-Host "⚠️  VieNeu-TTS Backend is already running on port 11111!" -ForegroundColor Yellow
    Write-Host "⚠️  VieNeu-TTS Backend đang chạy trên port 11111 rồi!" -ForegroundColor Yellow
...
Write-Host "Starting VieNeu-TTS Backend in background..." -ForegroundColor Cyan
Write-Host "Đang khởi động VieNeu-TTS Backend ở chế độ nền..." -ForegroundColor Cyan
...
try {
    ...
    Write-Host ""
    Write-Host "✅ VieNeu-TTS Backend started successfully!" -ForegroundColor Green
    Write-Host "✅ VieNeu-TTS Backend đã được khởi động thành công!" -ForegroundColor Green
    Write-Host ""
