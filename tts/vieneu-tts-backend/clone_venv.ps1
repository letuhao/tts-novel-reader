# Clone virtual environment from VieNeu-TTS
# Sao chép môi trường ảo từ VieNeu-TTS

Write-Host "🔄 Cloning VieNeu-TTS virtual environment..." -ForegroundColor Green
Write-Host "🔄 Đang sao chép môi trường ảo của VieNeu-TTS..." -ForegroundColor Green
Write-Host ""

$source_venv = "..\VieNeu-TTS\.venv"
$target_venv = ".\.venv"

# Check if source venv exists
# Kiểm tra xem venv nguồn có tồn tại
if (-not (Test-Path $source_venv)) {
    Write-Host "❌ Error: VieNeu-TTS venv not found at: $source_venv" -ForegroundColor Red
    Write-Host "❌ Lỗi: Không tìm thấy venv của VieNeu-TTS tại: $source_venv" -ForegroundColor Red
    exit 1
}
